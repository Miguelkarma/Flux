...
const Header = () => {
  ...
  return (
    ...
    >
      <div className="container backdrop-blur-xl rounded-full mx-auto h-full flex items-center justify-between  mt-5 shadow-md shadow-indigo-300 max-2xl:rounded-none max-2xl:mt-0  ">
        {/* Logo */}
        <Button
          className="text-2xl font-bold bg-gradient-to-t from-indigo-100 via-indigo-300 to-indigo-900 bg-clip-text text-transparent rounded-lg transition-all duration-300 hover:shadow-[2px_2px_0px_rgba(0,0,0,0.3)] hover:-translate-x-1 hover:-translate-y-1 hover:shadow-teal-200 hover:border hover:border-slate-600 shadow-none"
          onClick={() => {
            navigate("/");
          }}
        >
          ...
          <NavigationMenu className="text-sm">
            <NavigationMenuList className="flex items-center gap-1 relative z-10">
              {navMenu.map(({ href, label, icon }, index) => {
                const iconColors = ["text-indigo-300"];
                  ...
                  <NavigationMenuItem key={index} className="relative">
                    ...
                    >
                      ...
                      <motion.a
                        href={href || "#"}
                        onClick={(e) => handleNavigation(e, href || "#")}
                        className="flex items-center gap-2 px-4 py-2 absolute inset-0 z-10 bg-transparent text-muted-foreground text-indigo-300 transition-colors rounded-xl"
                      ...
                      >
                        {icon && (
                          <span
                            className={`transition-colors duration-300 text-indigo-300`}
                          >
                            {icon}
                          </span>
                        )}
        ...
        <div className="hidden lg:flex gap-2 !rounded-none hover:!rounded-full">
          ...
          <Button
            className="relative w-auto px-6 py-2 p-4 text-white bg-black border border-white/30 rounded-full transition-all hover:bg-black 
                before:absolute before:left-1/2 before:translate-x-[-50%] before:bottom-[-2px] before:w-[66%] before:h-[4px] before:bg-gradient-to-r before:from-transparent before:via-indigo-300 before:to-transparent hover:border-indigo-200 before:rounded-full"
            onClick={() => {
              navigate("/login");
            }}
          >
